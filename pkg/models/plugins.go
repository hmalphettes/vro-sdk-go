...
type Plugins struct {
	...
	// plugin
	Plugin []*PluginBundleMetadata `json:"plugin"`
		...
		if m.Plugin[i] != nil {
			if err := m.Plugin[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("plugin" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *Plugins) contextValidatePlugin(ctx context.Context, formats strfmt.Registry) error {
	...
	for i := 0; i < len(m.Plugin); i++ {
		...
		if m.Plugin[i] != nil {
			...
			if err := m.Plugin[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("plugin" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("plugin" + "." + strconv.Itoa(i))
